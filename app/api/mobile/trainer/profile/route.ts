...
export async function PUT(request: NextRequest) {
  return withErrorHandling(async () => {
    const account = await requireMobileTrainer(request)
    const payload = trainerUpdateSchema.pick({ fullName: true, phoneNumber: true, email: true, gender: true, specialty: true, availability: true }).parse(await request.json())
