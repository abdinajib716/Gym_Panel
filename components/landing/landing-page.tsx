  ...
  const renderSection = (key: string) => {
    ...
    if (key === "hero") return (
      <section key={key} id="home" className="landing-hero py-20 sm:py-24">
        ...
        <div className="landing-hero-media landing-reveal landing-reveal-5 w-full rounded-2xl border p-3 shadow-sm md:p-6"><div className="relative aspect-video overflow-hidden rounded-xl bg-muted p-3 md:p-6"><Image src="/images/misc/hero.png" alt="GymMaster dashboard preview" fill sizes="(max-width: 1024px) 100vw, 50vw" priority className="object-cover object-top dark:hidden" /><Image src="/images/misc/hero-dark.png" alt="GymMaster dashboard preview" fill sizes="(max-width: 1024px) 100vw, 50vw" priority className="hidden object-cover object-top dark:block" />{page.heroImage ? <Image src={page.heroImage} alt="GymMaster dashboard preview" fill sizes="(max-width: 1024px) 100vw, 50vw" priority className="object-cover object-top" /> : null}</div></div>
        </div>
      </section>
    )
    if (key === "about") return (
      <section key={key} id="about" className="landing-section container grid gap-8 py-14 sm:grid-cols-[1.1fr_0.9fr] sm:items-center sm:py-20">
        <div className="space-y-5"><p className="text-sm font-medium text-muted-foreground">ABOUT GYMMASTER</p><h2 className="text-3xl font-semibold tracking-tight sm:text-4xl">{page.aboutTitle}</h2><p className="max-w-xl text-base leading-7 text-muted-foreground">{page.aboutDescription}</p></div>
    ...
    if (key === "testimonials") return (
      <section key={key} id="testimonials" className="landing-section border-y bg-muted/40 py-14 sm:py-20"><div className="container space-y-9"><SectionTitle title="What our community says" description="Stories from the people who use GymMaster to make day-to-day operations easier." /><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{testimonials.length ? testimonials.map((item) => <article key={item.id} className="landing-card flex flex-col gap-5 rounded-lg border bg-card p-5 shadow-sm"><Quote className="size-5 text-muted-foreground" /><p className="flex-1 text-sm leading-6">{item.quote}</p><div className="flex items-center gap-3"><PersonImage src={item.image} name={item.name} /><div><p className="text-sm font-semibold">{item.name}</p>{item.role ? <p className="text-xs text-muted-foreground">{item.role}</p> : null}</div></div></article>) : <p className="col-span-full rounded-lg border border-dashed bg-background p-8 text-center text-sm text-muted-foreground">Member stories will appear here soon.</p>}</div></div></section>
    )
    if (key === "team") return (
    ...
    if (key === "contact") return (
      <section key={key} id="contact" className="landing-section border-t bg-muted/40 py-14 sm:py-20"><div className="container grid gap-8 lg:grid-cols-[0.8fr_1.2fr]"><div className="space-y-6"><div><p className="text-sm font-medium text-muted-foreground">CONTACT US</p><h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">Let&apos;s talk about your gym.</h2><p className="mt-3 text-base leading-7 text-muted-foreground">Have a question or want to see how GymMaster fits your operation? Send us a message.</p></div><div className="grid gap-4 text-sm">{page.contactEmail ? <a href={`mailto:${page.contactEmail}`} className="flex items-center gap-3"><Mail className="size-4 text-muted-foreground" />{page.contactEmail}</a> : null}{page.contactPhone ? <a href={`tel:${page.contactPhone}`} className="flex items-center gap-3"><Phone className="size-4 text-muted-foreground" />{page.contactPhone}</a> : null}{page.contactAddress ? <p className="flex items-center gap-3"><MapPin className="size-4 text-muted-foreground" />{page.contactAddress}</p> : null}</div></div><LandingContactForm /></div></section>
    )
    return null
  }
