...
import { CircleUserRound, Copy, CreditCard, Dumbbell, Eye, EyeOff, FileText, KeyRound, Mail, Plus, ShieldCheck, ShieldX, Trash2 } from "lucide-react"
...
export function CrudPage({
  ...
  const sectionStyles: Record<string, { icon: React.ComponentType<{ className?: string }>; iconClass: string }> = {
    "Member details": { icon: CircleUserRound, iconClass: "bg-[var(--brand-navy-soft)] text-[var(--brand-navy)]" },
    "Membership plan": { icon: Dumbbell, iconClass: "bg-primary/20 text-[var(--brand-navy)]" },
    "Manual payment": { icon: CreditCard, iconClass: "bg-destructive/10 text-destructive" },
  }
  const memberProfile = recordName === "member" && viewRecord ? {
    name: String(viewRecord.fullName ?? "Member"),
    image: String(viewRecord.profileImage ?? ""),
    contact: String(viewRecord.phoneNumber ?? viewRecord.email ?? "No contact information"),
    status: String(viewRecord.status ?? "PENDING"),
  } : null
  ...
  return (
    <div className="space-y-6">
      ...
      <AccessCard className={title === "Members" ? "members-directory" : undefined} title={`${title} Directory`} description={`Manage ${title.toLowerCase()} records.`}>
        <TableShell className={title === "Members" ? "members-table" : undefined}>
          <table className="w-full min-w-[760px] text-sm">
            ...
            <tbody>
              {records.map((record) => (
                <tr key={String(record.id)} className="data-table-row">
                  {allowDelete ? (
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-[96vw] max-w-[42rem] overflow-y-auto p-0">
          <SheetHeader className="border-b px-6 py-5">
            <SheetTitle>{editingRecord ? `Edit ${recordName}` : recordName === "member" ? "Register Member" : `Create ${recordName}`}</SheetTitle>
            <p className="text-sm text-muted-foreground">Fill in the module details and save changes.</p>
          </SheetHeader>
          <form onSubmit={handleSubmit} className="space-y-5 p-6">
            {fieldSections.map((section) => {
              const sectionStyle = sectionStyles[section.title] ?? { icon: FileText, iconClass: "bg-muted text-muted-foreground" }
              const SectionIcon = sectionStyle.icon
              const displayTitle = section.title.replace(/\b\w/g, (letter) => letter.toUpperCase())
              return (
              <AccessCard key={section.title} className="gym-form-section" title={<span className="flex items-center gap-3"><span className={`flex size-8 items-center justify-center rounded-lg ${sectionStyle.iconClass}`}><SectionIcon className="size-4" /></span>{displayTitle}</span>} description={section.title === "Details" ? `Core ${recordName} information.` : undefined}>
            ...
            <div className="flex items-center justify-between rounded-lg border bg-card px-5 py-4">
              ...
              <Button type="submit" className="gap-2" disabled={submitting}>
                ...
                {submitting ? "Saving..." : recordName === "member" && !editingRecord ? "Register Member" : "Save"}
              </Button>
            </div>
          </form>
        </SheetContent>
              ...
              <div className="rounded-2xl border border-border/70 bg-card p-5 shadow-sm">
                <div className="flex items-center gap-4">
                  <Avatar className="size-14 ring-2 ring-primary/25">
                    <AvatarImage src={memberProfile.image || undefined} alt={memberProfile.name} />
                    <AvatarFallback className="bg-primary/15 font-bold text-[var(--brand-navy)]">{memberProfile.name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-lg font-bold text-foreground">{memberProfile.name}</p>
                    <p className="mt-1 truncate text-sm text-muted-foreground">{memberProfile.contact}</p>
                    <div className="mt-2"><StatusPill value={memberProfile.status} /></div>
                  </div>
                </div>
              </div>
            ) : null}
