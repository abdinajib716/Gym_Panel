...
function getWaafiFailureReason(response: unknown, fallback: string) {
  if (!response || typeof response !== "object") return fallback
  const payload = response as {
    responseMsg?: unknown
    errorCode?: unknown
    params?: { description?: unknown; state?: unknown }
  }
  const description = typeof payload.params?.description === "string" ? payload.params.description : null
  const message = typeof payload.responseMsg === "string" ? payload.responseMsg : null
  const errorCode = payload.errorCode !== undefined && payload.errorCode !== null ? String(payload.errorCode) : null
  const state = payload.params?.state !== undefined && payload.params?.state !== null ? String(payload.params.state) : null

  return [
    description,
    message && message !== description ? message : null,
...
export async function initiateStoreWaafiPurchase(input: {
  ...
  // Keep Waafi identifiers in the same safe shape/length as the working
  // subscription payment flow. Longer store-prefixed values caused Waafi to
  // fail while creating its order before the EVC prompt was triggered.
  const referenceId = `ST-${requestId}`
  ...
  try {
    ...
    const failedTransaction = await prisma.storeTransaction.update({
    ...
    await createActivityLog({
      ...
      subjectId: failedTransaction.id,
      userDisplay: input.buyer.name,
      metadata: { buyerType: input.buyer.type, reason },
    })
    return { transaction: failedTransaction, order: null, product, paymentStatus: "FAILED" as const }
  }
  ...
  try {
    ...
    await createActivityLog({
      type: "store",
      activity: "Store payment request failed",
      subject: product.name,
      subjectId: failedTransaction.id,
      userDisplay: input.buyer.name,
      metadata: { buyerType: input.buyer.type, reason },
  ...
  const paymentStatus = waafiResponse.ok ? mapWaafiStatus(waafiResponse.body) : "FAILED"
  ...
  const failureReason = getWaafiFailureReason(waafiResponse.body, responseMessage || `WaafiPay payment ${paymentStatus.toLowerCase()}`)
  ...
  const updatedTransaction = await prisma.storeTransaction.update({
    ...
    data: {
      ...
      paymentStatus,
      failedReason: paymentStatus !== "PAID" ? failureReason : null,
      rawResponse: {
